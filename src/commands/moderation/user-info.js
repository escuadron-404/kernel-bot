...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addUserOption(option =>
            ...
            .setDescription('El miembro del que quieres ver la info (por defecto, tú mismo).')),
    ...
    async execute(interaction) {
        ...
        const userInfoEmbed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '📅 Cuenta Creada', value: `<t:${Math.floor(member.user.createdTimestamp / 1000)}:F> (<t:${Math.floor(member.user.createdTimestamp / 1000)}:R>)` },
                { name: '📥 Se Unió al Servidor', value: `<t:${Math.floor(member.joinedTimestamp / 1000)}:F> (<t:${Math.floor(member.joinedTimestamp / 1000)}:R>)` },
                { name: `🎭 Roles (${roles.length})`, value: roles.length > 0 ? roles.join(', ') : 'Ningún rol' }
            )
            .setTimestamp();
