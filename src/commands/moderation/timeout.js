...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('La razón para el timeout.'))
  ...
  async execute(interaction) {
    ...
    if (target.id === interaction.user.id) {
        return interaction.reply({ content: 'No puedes aplicarte un timeout a ti mismo.', ephemeral: true });
    }
    if (!durationMs) {
      return interaction.reply({ content: 'Por favor, proporciona una duración válida (ej: 5m, 2h, 3d).', ephemeral: true });
    }
    if (!target.moderatable) {
      return interaction.reply({ content: 'No puedo aplicar un timeout a este usuario. Puede que tenga un rol más alto que el mío.', ephemeral: true });
    }
