...
module.exports = {
	data: new SlashCommandBuilder()
		.setName("warn")
		.setDescription(
			"Añade una advertencia a un miembro (requiere base de datos).",
		)
	...
	async execute(interaction) {
		...
		if (
			...
			return interaction.reply({
				content:
					"❌ No puedes advertir a un miembro con un rol igual o superior al tuyo.",
				ephemeral: true,
			});
		}
		...
		const dmEmbed = new EmbedBuilder()
			...
			.setDescription(
				`Has recibido una advertencia en el servidor **${interaction.guild.name}**.`,
			)
			.addFields(
		...
		try {
			...
			console.log(
				`No se pudo enviar el MD de advertencia a ${target.user.tag}. Error: ${error}`,
			);
		}
