...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('id_usuario')
            .setDescription('La ID del usuario que quieres desbanear. No puedes usar una @mención.')
            .setRequired(true))
        .addStringOption(option =>
            option.setName('razón')
            .setDescription('La razón para el desbaneo.')),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            console.error(error);
            let errorMessage = '❌ Ocurrió un error al intentar desbanear al usuario.';
            if (error.code === 10013 || error.code === 10026) {
                errorMessage = '❌ No se pudo encontrar al usuario o no está baneado. Asegúrate de que la ID sea correcta.';
            }
