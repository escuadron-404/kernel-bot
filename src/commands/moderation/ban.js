...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) =>
			option.setName("razón").setDescription("La razón para el baneo."),
		)
	...
	async execute(interaction) {
		...
		if (!target.bannable) {
			return interaction.reply({
				content:
					"No puedo banear a este usuario. Puede que tenga un rol más alto que el mío.",
				ephemeral: true,
			});
		}
		...
		try {
			await target.send(
				`Has sido baneado del servidor **${interaction.guild.name}** por la siguiente razón: ${reason}`,
			);
		} catch (error) {
			console.log(
