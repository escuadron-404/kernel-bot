...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!target.manageable) {
            return interaction.reply({ content: '❌ No puedo cambiar el apodo de este usuario. Puede que tenga un rol superior al mío.', ephemeral: true });
        }
        if (interaction.member.roles.highest.position <= target.roles.highest.position) {
            return interaction.reply({ content: '❌ No puedes cambiar el apodo de un miembro con un rol igual o superior al tuyo.', ephemeral: true });
        }

        try {
            const oldNickname = target.displayName;
            await target.setNickname(newNickname, `Cambiado por ${interaction.user.tag}`);
            
            const nicknameEmbed = new EmbedBuilder()
                .setColor(0x7289DA)
                .setTitle('📝 Apodo Actualizado')
                .setDescription(newNickname ? `El apodo de **${oldNickname}** ha sido cambiado a **${newNickname}**.` : `El apodo de **${oldNickname}** ha sido reseteado.`)
                .addFields(
            ...
            console.error(error);
            await interaction.reply({ content: '❌ Hubo un error al intentar cambiar el apodo.', ephemeral: true });
        }
    },
};