...
module.exports = {
    ...
    async execute(interaction) {
        ...
        commands.forEach(cmd => {
            if (!cmd.category || cmd.category === 'owner') return; 
        ...
        if (categories['moderacion']) categories['moderacion'].emoji = '🛡️';
        if (categories['canal']) categories['canal'].emoji = '#️⃣';
        if (categories['utilidad']) categories['utilidad'].emoji = 'ℹ️';

        const homeEmbed = new EmbedBuilder()
            .setColor(0x5865F2)
            .setTitle('Centro de Ayuda')
            .setDescription('Usa el menú desplegable para explorar los comandos por categoría.')
            .setTimestamp();
