	...
	async execute(interaction) {
		try {
			...
			if (idValue) {
				...
				if (!found) {
					return interaction.reply(`❌ No se encontró un repositorio con ID \`${idValue}\`.`);
				}
				return interaction.reply(`🔍 **Repositorio encontrado:**\n\`${found.id}.\` [${found.categoria}] ${found.url}`);
			}
			...
			if (categoriaValue) {
				...
				if (repos.length === 0) {
					return interaction.reply(`📭 No hay repos para la categoría **${categoriaName}**.`);
				}
			}
			...
			const getPage = (page) => {
				...
				return repos.slice(start, end).map(repo => `\`${repo.id}.\` [${repo.categoria}] ${repo.url}`).join('\n');
			};
			...
			collector.on('collect', async i => {
				if (i.user.id !== interaction.user.id) return i.reply({ content: '⛔ No puedes usar estos botones.', ephemeral: true });
